...
namespace SoulsFormats
{
    ...
    public class HKXPWV : SoulsFile<HKXPWV>
    {
        ...
        {
            /// <summary>
            /// Disables collision of the ragdoll body with other characters
            /// </summary>
            public bool DisableCollision { get; set; }
            
            /// <summary>
            /// Disables attack hit detection for the ragdoll body
            /// </summary>
            public bool DisableHit { get; set; }
            ...
            /// <summary>
            /// Defines which NPC part group (if any) this ragdoll bone's collider is assigned to.
            /// Used for NPC part events in EMEVD. Weakspot is 31.
            /// </summary>
            public byte NPCPartGroupIndex { get; set; } = 0;

            /// <summary>
            /// Defines which NPC part group (if any) this ragdoll bone's collider is assigned to.
            /// Used for parts damage rate in params. Weakspot is 9.
            /// </summary>
            public byte NPCPartDamageGroup { get; set; } = 0;
            ...
            internal RagdollBoneEntry(BinaryReaderEx br, GameType game)
            {
                if (game == GameType.BB)
                {
                    ...
                    NPCPartDamageGroup = (byte)(bitfield & 0xf);
                    DisableCollision = ((bitfield >> 4) & 1) != 0;
                    DisableHit = ((bitfield >> 5) & 1) != 0;
            ...
            internal void Write(BinaryWriterEx bw, GameType game)
            {
                byte bitfield = NPCPartDamageGroup;
                bitfield |= (byte)(DisableCollision ? 1 << 4 : 0);
                bitfield |= (byte)(DisableHit ? 1 << 5 : 0);
                if (game == GameType.BB)
                {
                    bw.WriteInt32(RagdollParamID);
                    bw.WriteInt16(DamageAnimID);
                    bw.WriteByte(NPCPartGroupIndex);
                    bw.WriteByte(bitfield);
